...
const _prefsKey = 'user_files_list_v1';

final userFilesProvider =
    NotifierProvider<UserFilesNotifier, List<SoundItem>>(UserFilesNotifier.new);

class UserFilesNotifier extends Notifier<List<SoundItem>> {
  @override
  ...
  Future<void> _persist() async {
    ...
    await ref
        ...
        .pushSounds(items: state, recordings: false);
  }

  Future<void> _pullRemote() async {
    final remote = await ref
        .read(syncServiceProvider)
        .pullSounds(recordings: false);
  ...
  Future<void> remove(String id) async {
    ...
    if (item != null && !item.path.startsWith('assets/')) {
      try {
